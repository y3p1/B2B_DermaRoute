...
export async function listBvRequestsForProvider(providerId: string) {
  ...
  const rows = await db
    .select({
      ...
      deliveryAddress: bvRequests.deliveryAddress,
      deliveryCity: bvRequests.deliveryCity,
      deliveryState: bvRequests.deliveryState,
      deliveryZip: bvRequests.deliveryZip,
