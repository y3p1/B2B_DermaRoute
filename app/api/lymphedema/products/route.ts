import {
  listLymphedemaProductsController,
  createLymphedemaProductController,
...
export async function GET(request: Request) {
  return runServerPipeline(request, {
    ...
    handler: (req, res, next) => {
      void listLymphedemaProductsController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
...
export async function POST(request: Request) {
  return runServerPipeline(request, {
    ...
    handler: (req, res, next) => {
      void createLymphedemaProductController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
