import {
  getLymphedemaProductController,
  updateLymphedemaProductController,
  archiveLymphedemaProductController,
} from "../../../../../backend/controllers/lymphedemaProducts.controller";
...
export async function GET(request: Request) {
  return runServerPipeline(request, {
    ...
    handler: (req, res, next) => {
      void getLymphedemaProductController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
...
export async function PATCH(request: Request) {
  return runServerPipeline(request, {
    ...
    handler: (req, res, next) => {
      void updateLymphedemaProductController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
...
export async function DELETE(request: Request) {
  return runServerPipeline(request, {
    middlewares: [cors, baseRateLimit, requireAuth, requireAdminOrClinicStaff],
    handler: (req, res, next) => {
      void archiveLymphedemaProductController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
