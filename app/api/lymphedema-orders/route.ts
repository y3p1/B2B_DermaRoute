import {
  createLymphedemaOrderController,
  listLymphedemaOrdersController,
} from "../../../backend/controllers/lymphedema.controller";
...
export async function GET(request: Request) {
  return runServerPipeline(request, {
    ...
    handler: (req, res, next) => {
      void listLymphedemaOrdersController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
...
export async function POST(request: Request) {
  return runServerPipeline(request, {
    ...
    handler: (req, res, next) => {
      void createLymphedemaOrderController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
