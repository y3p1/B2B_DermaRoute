import {
  getOcularProductController,
  updateOcularProductController,
  archiveOcularProductController,
} from "../../../../../backend/controllers/ocularProducts.controller";
...
export async function GET(request: Request) {
  return runServerPipeline(request, {
    ...
    handler: (req, res, next) => {
      void getOcularProductController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
...
export async function PATCH(request: Request) {
  return runServerPipeline(request, {
    ...
    handler: (req, res, next) => {
      void updateOcularProductController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
...
export async function DELETE(request: Request) {
  return runServerPipeline(request, {
    middlewares: [cors, baseRateLimit, requireAuth, requireAdminOrClinicStaff],
    handler: (req, res, next) => {
      void archiveOcularProductController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
