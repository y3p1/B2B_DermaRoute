import { checkPolicyUrlController, deletePolicyMonitorController } from "@/backend/controllers/coveragePlans.controller";
...
export async function POST(request: Request) {
  return runServerPipeline(request, {
    ...
    handler: (req, res, next) => {
      void checkPolicyUrlController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
...
export async function DELETE(request: Request) {
  return runServerPipeline(request, {
    middlewares: [cors, baseRateLimit, requireAuth, requireAdmin],
    handler: (req, res, next) => {
      void deletePolicyMonitorController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
