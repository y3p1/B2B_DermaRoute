import {
  listInsuranceRoutesController,
  createInsuranceRouteController,
  bulkSetInsuranceRoutesController,
} from "@/backend/controllers/insuranceRouting.controller";
...
export async function GET(request: Request) {
  return runServerPipeline(request, {
    ...
    handler: (req, res, next) => {
      void listInsuranceRoutesController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
...
export async function POST(request: Request) {
  return runServerPipeline(request, {
    middlewares: [cors, baseRateLimit, requireAuth, requireAdminOrClinicStaff],
    handler: (req, res, next) => {
      void createInsuranceRouteController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
...
export async function PUT(request: Request) {
  return runServerPipeline(request, {
    middlewares: [cors, baseRateLimit, requireAuth, requireAdminOrClinicStaff],
    handler: (req, res, next) => {
      void bulkSetInsuranceRoutesController(req, res)
        .then(() => next())
        .catch(next);
    },
    errorHandler,
