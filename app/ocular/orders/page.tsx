...
export default function OcularOrdersPage() {
  ...
  const [orders, setOrders] = React.useState<OcularOrderRow[]>([]);
  ...
  const refresh = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiGet<{ success: true; data: OcularOrderRow[] }>("/api/ocular/orders", { token });
      setOrders(res.data);
    } catch (err) {
  ...
  const filtered = orders.filter((o) => {
    ...
    return name.includes(search.toLowerCase()) || (o.status ?? "").includes(search.toLowerCase());
  });
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-teal-900">Order History</h1>
          <p className="text-sm text-teal-600 mt-1">{orders.length} order{orders.length !== 1 ? "s" : ""} total</p>
        </div>
      ...
      <div className="mb-4">
        <input
          ...
          className="w-full max-w-sm px-3 py-2 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400 bg-white"
        />
      </div>
      ...
      <div className="bg-white rounded-xl border border-teal-100 shadow-sm overflow-hidden">
        {loading && orders.length === 0 ? (
          ...
          <div className="p-10 text-center text-slate-400 text-sm">
            {orders.length === 0
              ? <><p className="mb-3">No orders yet.</p><Link href="/ocular/orders/new" className="text-teal-600 underline font-medium">Place your first VisiDisc order</Link></>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                ...
                <th className="text-left px-5 py-3 font-medium text-slate-500">Qty</th>
