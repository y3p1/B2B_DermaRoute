...
type GarmentChoice = { label: string; category: string; image: StaticImageData; value: string };
...
interface LymphedemaOrderModalProps {
  open: boolean;
...
function getRecommendedDevice(formData: FormData): { device: string; hcpcs: string } {
  const hasArm = formData.extremity.some((e) => ARM_EXTREMITIES.has(e));
  const isAdvantageOrCommercial = formData.insurance === "Medicare Advantage" || formData.insurance === "Commercial";
  if (hasArm || isAdvantageOrCommercial) return { device: "AIROS 8", hcpcs: "E0652" };
...
export function LymphedemaOrderModal({ open, onOpenChange, onCreated }: LymphedemaOrderModalProps) {
  ...
  // Auto-fill device recommendation when entering step 2
  React.useEffect(() => {
  ...
  const handleBack = () => { if (step > 1) setStep((s) => s - 1); setError(null); };
      ...
      <DialogContent className="max-w-2xl p-0 gap-0 overflow-hidden">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-slate-100">
          <div className="flex items-center gap-2 mb-4">
            <div className="flex items-center justify-center w-8 h-8 rounded-lg" style={{ background: "oklch(0.92 0.06 160)" }}>
              <Wind className="w-4 h-4" style={{ color: "oklch(0.45 0.12 160)" }} />
            </div>
            <DialogTitle className="text-base font-semibold text-slate-800">New Lymphedema Order</DialogTitle>
          </div>
              ...
              return (
                <React.Fragment key={num}>
                  <div className="flex items-center gap-2">
                    <div
                      className={cn("w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-colors", done || active ? "text-white" : "bg-slate-100 text-slate-400")}
                      style={done || active ? { background: "oklch(0.45 0.12 160)" } : undefined}
                    >
                    ...
                    <div className="flex gap-4 items-start">
                      ...
                      {selectedGarmentImage && (
                        <div className="w-[120px] h-[120px] border border-slate-200 rounded-lg overflow-hidden flex items-center justify-center bg-white shrink-0">
                          <NextImage src={selectedGarmentImage} alt={formData.garmentStyle} width={120} height={120} className="object-contain" />
                        </div>
                      )}
                    </div>
                    <FieldRow>
            ...
            {step === 3 && (
              <div className="space-y-5">
                ...
                <div className="rounded-xl border border-slate-200 p-4 space-y-1">
                  ...
                  <SummaryRow label="Device recommended" value={formData.deviceRecommended ? "Yes" : "Overridden by staff"} />
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 bg-slate-50">
          <Button variant="outline" size="sm" onClick={step === 1 ? () => onOpenChange(false) : handleBack} disabled={isSubmitting} className="gap-1.5">
            {step === 1 ? "Cancel" : <><ChevronLeft className="w-4 h-4" />Back</>}
          </Button>
          {step < 3 ? (
            <Button size="sm" onClick={handleNext} disabled={step === 1 ? !step1Valid : !step2Valid} className="gap-1.5 text-white" style={{ background: "oklch(0.45 0.12 160)" }}>
              Next <ChevronRight className="w-4 h-4" />
            </Button>
          ) : (
            <Button size="sm" onClick={() => { void handleSubmit(); }} disabled={isSubmitting} className="gap-1.5 text-white" style={{ background: "oklch(0.45 0.12 160)" }}>
