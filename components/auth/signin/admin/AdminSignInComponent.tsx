  ...
  const handleSendCode = async (values: z.infer<typeof phoneSchema>) => {
    ...
    try {
      ...
      await apiPost<
        ...
        { phone: string; mode: "signin"; role?: "admin" }
      >("/api/send-otp", {
        phone: values.phone,
        mode: "signin",
        role: "admin",
      ...
      if (err instanceof ApiError) {
        if (err.status === 404 && err.message === "Phone is not registered") {
          setError({
            ...
            description:
              "We couldn't find an admin account for this phone number. Please register or try a different number.",
            cta: { label: "Register", href: "/admin/signup" },
          });
          return;
        }
        ...
        {stage === "enterPhone" ? (
          ...
          <VerifySignIn
            ...
            onVerify={async (code) => {
              ...
              setError(null);
              try {
                const phone = submittedPhone || phoneForm.getValues("phone");
                ...
                if (after.status !== "authenticated") {
                  setError({
                    ...
                    description:
                      after.error ||
                      "Signed in successfully, but your account is not authorized as an admin.",
                  });
                  return;
                }
                ...
                console.log("after.role:", after.role);
