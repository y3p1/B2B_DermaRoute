...
export type AdminProfileForm = z.infer<typeof profileSchema>;
...
export interface AdminSignupStep1Props {
  onNext: (data: AdminProfileForm) => void;
  defaultValues?: Partial<AdminProfileForm>;
}

export default function AdminSignupStep1Profile({
  onNext,
  defaultValues,
}: AdminSignupStep1Props) {
  ...
  const handleSubmit = async (values: AdminProfileForm) => {
    if (isClientDemoMode()) {
      setSubmitError({
        title: "Demo mode",
